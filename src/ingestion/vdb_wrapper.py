...
class LoadInVdb:
    ...
    def add_to_collection(self,
        ...
        if not (len(dense_vectors) == len(sparse_vectors) == len(payloads) == len(ids)):
            raise ValueError('ids, dense vector, sparse vector and payloads lists must have the same length')
        ...
        self.client.upload_points(
            ...
            points=[
                models.PointStruct(
                    ...
                    payload=payload
                ) for a_id, dense_vector, sparse_vector, payload in zip(ids, dense_vectors, sparse_vectors, payloads)
            ],
            max_retries=3
        )
        print(f'Number of elements in {self.coll_name}: {self.client.count(collection_name=self.coll_name)}')
